var my={}
function steerMain(){let version='0.5';my.wd=600
my.ht=500
my.playQ=true
my.frame=0
my.car=new Car();my.imgs=[{file:'BMW-Z4.svg',lt:25,tp:38,wd:160},{file:'glibersat.svg',lt:28,tp:36,wd:160},]
my.img=my.imgs[0]
my.zone={wd:200,ht:100}
let s="";s+=`<div style="position: relative; width: ${my.wd}px; height: ${my.ht}px; text-align: center; margin:auto;
  border-radius:10px; box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;">`
s+='<canvas id="canTrack" style="position: absolute; width:0; height:0; left: 0px; top: 0px; z-index: 3; "></canvas>';my.imgHome=(document.domain=='localhost')?'/mathsisfun/games/images/':'/games/images/'
...
s+='<canvas id="canBd" style="position: absolute; width:0; height:0; left: 0px; top: 0px; z-index: 5; "></canvas>';s+='<div style="font: 11px Arial; position: absolute; left: 0px; bottom: 0px; color: #6600cc;">&copy; 2021 MathsIsFun.com  v'+version+'</div>';s+='</div>';document.write(s);my.canTrack=canvasInit('canTrack',my.wd,my.ht,2)
my.canBd=canvasInit('canBd',my.wd,my.ht,2)
my.rotLtQ=false;my.rotRtQ=false;my.speedUpQ=false;my.speedDnQ=false;my.brakeQ=false;document.onkeydown=function(e){e.preventDefault();switch(e.keyCode){case 37:my.rotLtQ=true;break;case 39:my.rotRtQ=true;break;case 38:my.speedUpQ=true;break;case 40:my.speedDnQ=true;break;case 32:my.brakeQ=true;break;case 78:polyNext()
break;default:console.log('',e.keyCode)}};document.onkeyup=function(e){switch(e.keyCode){case 37:my.rotLtQ=false;break;case 39:my.rotRtQ=false;break;case 38:my.speedUpQ=false;break;case 40:my.speedDnQ=false;break;case 32:my.brakeQ=false;}};window.addEventListener("mousedown",function(ev){console.log('['+ev.clientX+','+ev.clientY+']');},false)
my.polys=[[[287,60],[451,60],[451,147],[287,147]],[[36,140],[217,139],[220,252],[107,224],[38,239]],[[472,10],[580,10],[580,273],[472,273]],[[127,292],[278,327],[181,472],[115,453]],[[441,32],[299,177],[529,202]],[[100,200],[100,340],[200,340],[200,200]],[[12,11],[160,11],[160,105],[12,105]],[[430,200],[340.2,323.6],[194.8,276.4],[194.8,123.6],[340.2,76.36]],[[213,251],[288,187],[405,311],[333,374]],[[346,397],[455,290],[526,349],[522,458]],]
my.poly=new Poly()
my.polyN=-1
polyNext()
go()
anim()}
function go(){my.car.move();my.car.draw();my.frameN=0}
function anim(){this.frame++;loop();if(my.playQ&&my.frameN++<500000)
requestAnimationFrame(anim);}
function loop(){my.car.update()
my.car.move();my.car.draw()
my.car.wheelsDraw()}
...
if(this.prev.lt!=lt||this.prev.tp!=tp||this.prev.rot!=rot){if(my.frame++>10){my.frame=0
let g=my.canTrack.g
g.fillStyle="rgba(255,255,255,0.02)";g.fillRect(0,0,g.canvas.width,g.canvas.height);}
...
console.log('in:',inQ,inCanQ,this.vel.mag)}
let g=my.canTrack.g
...
my.poly.drawLines(g)
g.fill();g.stroke()}
function ptsRotate(pts,midX,midY,rot){var newPts=[];for(var i=0;i<pts.length;i++){var pt=pts[i];newPts.push(pt.rotate(rot).add(midX,midY));}
return newPts;}
...
class Pt{constructor(x,y){this.x=x;this.y=y;this.name="?";this.rad=12;}
add(dx,dy){return new Pt(this.x+dx,this.y+dy);}
rotate(angle){var cosa=Math.cos(angle);var sina=Math.sin(angle);var xPos=this.x*cosa+this.y*sina;var yPos=-this.x*sina+this.y*cosa;return new Pt(xPos,yPos);}
getAvg(pts){let xSum=0;let ySum=0;console.log('pts',pts)
for(let i=0;i<pts.length;i++){xSum+=pts[i].x;ySum+=pts[i].y;}
...
function ptsDraw(g,pts,closeQ){closeQ=typeof closeQ!=='undefined'?closeQ:false;for(var i=0;i<pts.length;i++){if(i==0){g.moveTo(pts[i].x,pts[i].y);}else{g.lineTo(pts[i].x,pts[i].y);}}
...
let px=new Pt(pt.x,pt.y);this.pxs.push(px);}}else{for(let i=0;i<this.pts.length;i++){let pt=this.pts[i]
